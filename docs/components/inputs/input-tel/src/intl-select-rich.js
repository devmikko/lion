...
class IntlSelectInvoker extends LionSelectInvoker {
  ...
  get _templateData() {
    return {
      data: { selectedElement: this.selectedElement, selectRichElement: this.selectRichElement },
    };
  }
  ...
  static _contentTemplate({ data: { selectedElement, selectRichElement } }) {
    if (!selectedElement) {
      return ``;
    }
    return html`
      <div class="iti__flag iti__${selectedElement._regionCode?.toLowerCase()}"></div>
      <div class="iti__arrow iti__arrow--${selectRichElement.opened ? 'up' : 'down'}"></div>
    `;
  }
}
